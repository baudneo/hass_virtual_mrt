...
from .const import DOMAIN, CONF_ROOM_PROFILE, ROOM_PROFILES, CONF_THERMAL_ALPHA
...
class VirtualFactorNumber(RestoreNumber):
    ...
    def __init__(
        ...
        self._key = key
        # Stable Unique ID based on the config entry ID
        self._attr_unique_id = f"{entry.entry_id}_{key}"
        # self._attr_name = translation_key
        self.translation_key = translation_key
        self._attr_device_info = device_info  # Link to the device
        self._icon = icon
...
class VirtualThermalAlphaNumber(RestoreNumber):
    """A number entity for setting the smoothing factor (alpha). Lower = high thermal mass (Masonry walls). Higher = low thermal mass (Wood/Drywall walls)."""
    ...
    # Note: We don't need to implement set_value/set_native_value on the number platform itself
    # as the RestoreNumber base class provides the async functionality.
